...
export default function OrderConfirmed(props) {
    ...
    const order_obj = location.state.props
    ...
    const get_order = async () => {
        const response = await fetch(props.url + 'orders_api/order-page/'+order_obj.order_id, {
            credentials: 'include',
    ...
    if (loading) {
        item_list = order.order_products.map(item => {
